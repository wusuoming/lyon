...
public abstract class BaseInterceptor implements Interceptor, Serializable {
    ...
    protected static Page convertParameter(Object parameterObject, Page pageVO)
        ...
        else if (parameterObject instanceof MapperMethod.MapperParamMap<?>) {
            @SuppressWarnings("unchecked")
            MapperMethod.MapperParamMap<Object> paramMap = (MapperMethod.MapperParamMap<Object>) parameterObject;
            // 如果多参数，分页参数要放到第一个位置
            parameterObject = paramMap.get("0");
            return convertParameter(parameterObject, pageVO);
        }
        else {
            // 参数为某个实体，该实体拥有Page属性
            Paging paging = parameterObject.getClass().getAnnotation(
                Paging.class);
            String field = paging.field();
            Field pageField = Reflections.getAccessibleField(parameterObject,
                field);
            if (pageField != null) {
                pageVO = (Page) Reflections.getFieldValue(parameterObject,
                    field);
                if (pageVO == null)
