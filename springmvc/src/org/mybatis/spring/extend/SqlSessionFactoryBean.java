 ...
 */
package org.mybatis.spring.extend;
 ...
 * demarcation in combination with a {@code SqlSessionFactory}. JTA should be
 * used for transactions
 * which span multiple databases or when container managed transactions (CMT)
 * are being used.
 * 
...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>,
    ...
    // 扩展
    private String extTypeAliasesPackage;

    private String mapperPackage;

    public String getExtTypeAliasesPackage() {
        return extTypeAliasesPackage;
    }

    public void setExtTypeAliasesPackage(String extTypeAliasesPackage) {
        this.extTypeAliasesPackage = extTypeAliasesPackage;
    }
     ...
     * MyBatis config file.
     * This property being based on Spring's resource abstraction also allows
     * for specifying
     ...
     * {@code SqlSessionFactory}: for example, you could specify the same
     ...
     * {@code DataSource} to manage transactions for, not
     * a {@code TransactionAwareDataSourceProxy}. Only data access code may work
     * with {@code TransactionAwareDataSourceProxy}, while the transaction
     * manager needs to work on the
     * underlying target {@code DataSource}. If there's nevertheless a
     * {@code TransactionAwareDataSourceProxy} passed in, it will be unwrapped
     * to extract its target {@code DataSource}.
     * 
     */
     ...
     * can be injected. By
     * default, {@code SqlSessionFactoryBuilder} creates
     ...
     * for all cases:
     * be it Spring transaction management, EJB CMT or plain JTA. If there is no
     * active transaction,
     * SqlSession operations will execute SQL statements non-transactionally.
     * 
     * <b>It is strongly recommended to use the default
     * {@code TransactionFactory}.</b> If not used, any
     * attempt at getting an SqlSession through Spring's MyBatis framework will
     ...
     * have set in the MyBatis
     * config file. This is used only as a placeholder name. The default value
     * is {@code SqlSessionFactoryBean.class.getSimpleName()}.
     * 
     * @param environment
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
        ...
        parseMapperPackage(configuration);
            ...
            for (Resource mapperLocation : this.mapperLocations) {
                ...
                try {
                    XMLMapperBuilder xmlMapperBuilder = new XMLMapperBuilder(
                        ...
                        configuration, mapperLocation.toString(), configuration
                            .getSqlFragments());
    ...
    private void parseExtTypeAliases(Configuration configuration) {
        ...
        try {
            for (String ext : extArr) {
                Resource[] resources = resolver.getResources(prefix + ext
                    + suffix);
                if (resources != null) {
    ...
    // 解析扩展的mapperPackage配置
    private void parseMapperPackage(Configuration configuration)
        throws IOException {
        ...
        ResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
        for (String ext : extArr) {
            Resource[] mapperResources = resolver.getResources(prefix + ext
                + suffix);
            for (Resource resource : mapperResources) {
                ...
                try {
                    XMLMapperBuilder xmlMapperBuilder = new XMLMapperBuilder(
                        ...
                        configuration, resource.toString(), configuration
                            .getSqlFragments());
                    xmlMapperBuilder.parse();
                }
                catch (Exception e) {
